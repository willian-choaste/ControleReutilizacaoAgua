...
#define BLYNK_AUTH_TOKEN "mkM9VyroNS_FMWulo8Y7WGU9a5e-TxhO"
...
#define valvulaHorta 26       // Eletroválvula horta
#define valvulaSanitario 25   // Eletroválvula sanitario
#define valvulaLimpeza 33     // Eletroválvula limpeza
#define valvulaEsgotamento 32 // Eletroválvula esgotamento
...
void controleTds() { // Verifica os niveis de TDS (Total de Sólidos Diluidos) na água
  ...
  voltagem = leituraSensorTds*3.3/4095; //Converte a leitura analógica em voltagem
  valorTds=(133.42*voltagem*voltagem*voltagem - 255.86*voltagem*voltagem + 857.39*voltagem)*0.5; //Converte o valor da tensão para o valor TDS
  ...
  //  Blynk.virtualWrite(V12, 0);       // Escreve o valor 0 no pino virtual V12 (led visualização no aplicativo do modo manual físico desatuado)
    ...
    if(flagFaltaAguaCaixa == false){          // Verifica se há água na caixa
      if (valorTds <= 100){                   // Verifica se o valor da variavel valorTds está entre 0 e 100, o que significa alta pureza
        ...
        Serial.print("Valor TDS (Total deSólidos Diluidos), não aceitável!!!\n");
